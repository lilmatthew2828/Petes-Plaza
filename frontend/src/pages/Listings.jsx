...
if (Number(price) < 0) {
  ...
  try {
    setLoading(true); // show loading state while uploading image

    // 1) Upload the image file to FastAPI → S3
    const { image_key: uploadedKey } = await uploadImage(image_key);
    console.log("UPLOADED KEY:", uploadedKey);

    // 2) Create the listing using the S3 key
    ...
    setLoading(false); // hide loading state after everything is done
  }
};
